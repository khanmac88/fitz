...
import { Alert, Dimensions, Image, TouchableOpacity, BackgroundImage } from 'react-native';
	...
	render() {
		return (
			<Container style={{ backgroundColor: 'black' }}>
				...
				<Header style={{ backgroundColor: 'black', borderBottomWidth: 0, shadowOpacity: 0, elevation: 0, }}>
					<Left style={{ flex: 1 }}>
						<Button transparent>
							<SimpleLineIcons name='arrow-left' style={{ fontSize: 18, color: '#808080' }} onPress={() => this.props.navigation.goBack()} />
						</Button>
					</Left>
					<Body style={{ flex: 4, justifyContent: 'center', alignItems: 'center' }}>
						<Title style={{ color: '#808080' }}>{Strings.ST29}</Title>
					</Body>
					<Right style={{ flex: 1 }} />
				</Header>
				<Body>
					<KeyboardAwareScrollView>
						...
						<View style={{ flex: 0.8, flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: 20, marginTop: 30 }}>
							...
							<Button rounded block onPress={this.restpass.bind(this)} style={styles.button_auth}>
								<Text style={{ color: '#808080' }}>{Strings.ST28}</Text>
							</Button>
